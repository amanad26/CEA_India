package com.example.cea.Utills.searchablespinner
...
class SearchableSpinner(private val context: Context) : LifecycleObserver {
    ...
    private fun AlertDialog.initView() {
        ...
        if (windowTitle != null || windowTitleVisibility.visibility == SearchView.VISIBLE) {
            dialogView.findViewById<TextView>(R.id.textViewTitle).visibility = View.VISIBLE
            dialogView.findViewById<TextView>(R.id.textViewTitle).text = windowTitle
            dialogView.findViewById<TextView>(R.id.textViewTitle).setTextColor(windowTitleTextColor)
        } else dialogView.findViewById<TextView>(R.id.textViewTitle).visibility = windowTitleVisibility.visibility

        //init SearchView
        if (searchViewVisibility.visibility == SearchView.VISIBLE) {
            dialogView.findViewById<SearchView>(R.id.searchView).queryHint = searchQueryHint
            dialogView.findViewById<SearchView>(R.id.searchView).setOnQueryTextListener(object : SearchView.OnQueryTextListener {
        ...
        } else dialogView.findViewById<SearchView>(R.id.searchView).visibility = searchViewVisibility.visibility
        ...
        if (negativeButtonVisibility.visibility == SearchView.VISIBLE) {
            ...
            dialogView.findViewById<Button>(R.id.buttonClose).text = negativeButtonText
            dialogView.findViewById<Button>(R.id.buttonClose).setTextColor(negativeButtonTextColor)
        } else dialogView.findViewById<Button>(R.id.buttonClose).visibility = negativeButtonVisibility.visibility

        //set Recycler Adapter
        if (::recyclerAdapter.isInitialized) {
            recyclerAdapter.highlightSelectedItem = highlightSelectedItem
            dialogView.findViewById<RecyclerView>(R.id.recyclerView).adapter = recyclerAdapter
        }
    }

    private fun initDialogColorScheme() {
        if (windowTopBackgroundColor != null)
            dialogView.findViewById<RelativeLayout>(R.id.headLayout).background = ColorDrawable(windowTopBackgroundColor!!)
        if (negativeButtonBackgroundColor != null)
            dialogView.findViewById<Button>(R.id.buttonClose).backgroundTintList =
    ...
    private fun toggleKeyBoard(showKeyBoard: Boolean) {
        ...
        if (showKeyboardByDefault && showKeyBoard) {
            dialogView.findViewById<SearchView>(R.id.searchView).post {
                dialogView.findViewById<SearchView>(R.id.searchView).requestFocus()
